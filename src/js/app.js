...
class App {
    constructor() {
        ...
        this.interviewPanel = null; // Lazily initialised in init()
    }
    ...
    async init() {
        ...
        // Initialise Interview Assistant (frontend-only, no backend changes).
        this._initInterviewAssistant();

        console.log('🌐 Translator + Interview Assistant v0.6.0 initialized');
    }

    _initInterviewAssistant() {
        try {
            ...
            // Settings form bindings (Interview tab).
            this._bindInterviewSettingsForm();
        } catch (e) {
            console.error('[App] Interview Assistant init failed:', e);
        }
    }

    async _openExternalUrl(url) {
        try {
            // Tauri 2 opener plugin
            ...
            console.warn('[App] Tauri open failed, falling back to window.open:', e);
        }
        window.open(url, '_blank');
    }

    _bindInterviewSettingsForm() {
        const sync = () => {
            const s = interviewSettingsStorage.get();
            const map = {
                'iv-set-enabled': 'enabled',
                'iv-set-auto-detect': 'autoDetectQuestion',
                'iv-set-combined-mode': 'combinedMode',
                'iv-set-default-browser': 'openInDefaultBrowser',
                'iv-set-auto-copy': 'autoCopyGeneratedPrompt',
                'iv-set-save-prompts': 'savePromptsToHistory',
                'iv-set-save-answers': 'savePastedAnswersToHistory',
            };
            for (const [id, key] of Object.entries(map)) {
                const el = document.getElementById(id);
                if (el && typeof s[key] === 'boolean') el.checked = !!s[key];
            }
            const setVal = (id, val) => {
                const el = document.getElementById(id);
                if (el && val !== undefined) el.value = val;
            };
            setVal('iv-set-debounce', s.detectionDebounceMs);
        ...
        sync();
        interviewSettingsStorage.onChange(sync);

        const bindBool = (id, key) => {
            document.getElementById(id)?.addEventListener('change', (e) => {
                interviewSettingsStorage.update({ [key]: !!e.target.checked });
            });
        };
        bindBool('iv-set-enabled', 'enabled');
        bindBool('iv-set-auto-detect', 'autoDetectQuestion');
        bindBool('iv-set-combined-mode', 'combinedMode');
        bindBool('iv-set-default-browser', 'openInDefaultBrowser');
        bindBool('iv-set-auto-copy', 'autoCopyGeneratedPrompt');
        bindBool('iv-set-save-prompts', 'savePromptsToHistory');
        bindBool('iv-set-save-answers', 'savePastedAnswersToHistory');

        const bindNum = (id, key) => {
            document.getElementById(id)?.addEventListener('change', (e) => {
                const v = Number(e.target.value);
                if (!Number.isFinite(v)) return;
                interviewSettingsStorage.update({ [key]: v });
            });
        };
        bindNum('iv-set-debounce', 'detectionDebounceMs');
        bindNum('iv-set-duplicate', 'duplicateThreshold');
        bindNum('iv-set-minlen', 'minQuestionLength');

        document.getElementById('iv-set-chatgpt-url')?.addEventListener('change', (e) => {
    ...
    _bindEvents() {
        ...
        // Back from interview
        document.getElementById('btn-interview-back')?.addEventListener('click', () => {
