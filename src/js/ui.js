...
export class TranscriptUI {
    constructor(container) {
        ...
        // Subscribers notified whenever the transcript content changes.
        // Used by the Interview Assistant's auto-detect pipeline.
        this._textUpdateListeners = new Set();
    }

    /**
     * Subscribe to transcript-text changes. Returns an unsubscribe fn.
     * The callback receives `{ kind, text, language }` where kind is one of
     * 'original' | 'translation' | 'provisional'.
     */
    onTextUpdate(cb) {
        if (typeof cb !== 'function') return () => {};
        this._textUpdateListeners.add(cb);
        return () => this._textUpdateListeners.delete(cb);
    }

    _emitTextUpdate(kind, text, language) {
        if (!this._textUpdateListeners.size) return;
        for (const cb of this._textUpdateListeners) {
            try { cb({ kind, text: text || '', language: language || null }); }
            catch (e) { console.error('[TranscriptUI] listener error:', e); }
        }
    }
    ...
    addOriginal(text, speaker, language) {
        ...
        this._render();
        this._emitTextUpdate('original', text, language);
    }
    ...
    addTranslation(text) {
        ...
        this._render();
        this._emitTextUpdate('translation', text, null);
    }
    ...
    setProvisional(text, speaker, language) {
        ...
        this._render();
        this._emitTextUpdate('provisional', text, language);
    }
